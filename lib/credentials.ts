...
export function requiresPatientSignature(credentials: Credentials | null | undefined): boolean {
...
export function hasFullClinicalAccess(credentials: Credentials | null | undefined): boolean {
  if (!credentials) return false;
  return FULL_CLINICAL_ACCESS.includes(credentials);
}

// Helper function to get display label
export function getCredentialsLabel(credentials: Credentials | null | undefined, short: boolean = false): string {
  if (!credentials) return 'Unknown';
  return short ? CREDENTIALS_SHORT_LABELS[credentials] : CREDENTIALS_LABELS[credentials];
}
