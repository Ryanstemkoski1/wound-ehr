...
async function testAllUsers() {
  ...
  users.forEach(user => {
    ...
    console.log(`   Roles: ${userRoles.map(r => `${r.role}${r.facility_id ? ` @ ${facilityMap[r.facility_id] || 'Unknown'}` : ''}`).join(', ') || 'None'}`);
    console.log(`   Facilities: ${userFacilities.map(f => facilityMap[f.facility_id] || 'Unknown').join(', ') || 'None'}`);
    console.log('');
  });
  ...
  if (procedures) {
    ...
    console.log(`Restricted Procedures (require MD/DO/PA/NP): ${restrictedProcs.length}\n`);
    ...
    users.forEach(user => {
      if (!user.credentials) {
        console.log(`❌ ${user.name}: No credentials set - Cannot perform any procedures`);
        return;
      }
      ...
      const deniedProcs = procedures.filter(p => 
        !p.allowed_credentials?.includes(user.credentials)
      );

      console.log(`${deniedProcs.length > 0 ? '⚠️' : '✅'} ${user.name} (${user.credentials})`);
      ...
      if (deniedProcs.length > 0) {
        console.log(`   🚫 Blocked codes: ${deniedProcs.map(p => p.procedure_code).join(', ')}`);
      }
      console.log('');
    });
  }
  ...
  const credentialTypes = [...new Set(users.map(u => u.credentials).filter(Boolean))];
