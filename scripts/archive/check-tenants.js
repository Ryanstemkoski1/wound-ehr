...
async function checkTenants() {
  ...
  const { data: roles } = await supabase.from('user_roles').select('user_id, tenant_id, role, facility_id');
  ...
  users.forEach(u => {
    ...
    if (userRoles.length === 0) {
      ...
      userRoles.forEach(role => {
        ...
        console.log(`✅ ${u.name}: ${tenant?.name || 'Unknown'} (${role.role})`);
      });
    }
  });
  
  console.log('\n=== ISSUE FOUND ===');
  const ryanTenant = roles.find(r => users.find(u => u.id === r.user_id && u.email === 'ryan.stemkoski@abectech.com'))?.tenant_id;
