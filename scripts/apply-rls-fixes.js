...
async function applyFixes() {
  ...
  await executeSql(
    `DROP POLICY IF EXISTS "Admins can view invites in their tenant" ON user_invites;`,
    "Drop old policy"
  );
  ...
  console.log("\n⚠️  IMPORTANT NOTES:");
  console.log(
    "1. user_roles table has RLS DISABLED by design (to avoid recursion)"
  );
  console.log(
    "2. Always use get_user_role_info() RPC function for role checks"
  );
