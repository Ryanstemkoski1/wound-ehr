...
async function testAddendums() {
  ...
  try {
    ...
    if (!visits || visits.length === 0) {
      logTest('Find signed visit', false, 'No signed visits found - create one first');
      return;
    }
    ...
    if (readError) {
      ...
      logTest('Read addendum', true, `Note: ${readAddendum.note.substring(0, 50)}...`);
    }
    
    // Test 3: Use RPC function to get addendums with user data
    const { data: rpcAddendums, error: rpcError } = await supabase
    ...
    if (rpcError) {
      ...
      if (foundAddendum && foundAddendum.users) {
        ...
        const userData = Array.isArray(foundAddendum.users) ? foundAddendum.users : JSON.parse(foundAddendum.users);
        logTest('RPC get_visit_addendums', true, `Found addendum with user: ${userData[0]?.full_name || 'Unknown'}`);
      } else {
        logTest('RPC get_visit_addendums', false, 'Addendum found but no user data');
      }
    }
    ...
    if (updateError) {
      ...
      logTest('Update addendum count', true, `Count: ${allAddendums?.length || 0}`);
    }
  ...
  try {
    ...
    if (patients && patients.length > 0) {
      ...
      if (detailsError) {
        ...
        logTest('Get patient details', true, `${details.first_name} ${details.last_name} - ${details.facility?.name || 'No facility'}`);
      }
      ...
      if (woundsError) {
        ...
        logTest('Get patient wounds', true, `Found ${wounds?.length || 0} wounds`);
      }
    }
...
async function testVisitsAndSignatures() {
  ...
  try {
    ...
    if (visits && visits.length > 0) {
      ...
      const signedVisits = visits.filter(v => v.status === 'signed' || v.status === 'submitted');
      logTest('Signed visits', true, `${signedVisits.length} of ${visits.length} are signed/submitted`);
      ...
      if (signedVisits.length > 0) {
        ...
        const { data: visitDetails, error: detailsError } = await supabase
          .from('visits')
          .select('*, provider_signature:signatures!provider_signature_id(*), patient_signature:signatures!patient_signature_id(*)')
          .eq('id', visit.id)
          .single();
        
        if (detailsError) {
          logTest('Get signature data', false, `Error: ${detailsError.message}`);
        } else {
          const hasProvider = !!visitDetails.provider_signature;
          const hasPatient = !!visitDetails.patient_signature;
          logTest('Get signature data', true, `Provider: ${hasProvider}, Patient: ${hasPatient}`);
        }
      }
    }
...
async function testAssessments() {
  ...
  try {
    ...
    logTest('List assessments', true, `Found ${assessments?.length || 0} assessments`);
    ...
    if (assessments && assessments.length > 0) {
      ...
      const { data: details, error: detailsError } = await supabase
        .from('assessments')
        .select('*, wound:wounds(location, wound_type), visit:visits(visit_date)')
        .eq('id', assessment.id)
        .single();
      
      if (detailsError) {
        logTest('Get assessment details', false, `Error: ${detailsError.message}`);
      } else {
        logTest('Get assessment details', true, `Wound: ${details.wound?.location}, Date: ${details.visit?.visit_date}`);
      }
      ...
      if (photosError) {
        logTest('Get assessment photos', false, `Error: ${photosError.message}`);
      } else {
        logTest('Get assessment photos', true, `Found ${photos?.length || 0} photos`);
      }
    }
...
async function testBilling() {
  ...
  try {
    ...
    logTest('List billing records', true, `Found ${billings?.length || 0} billing records`);
    ...
    if (billings && billings.length > 0) {
      ...
      const { data: details, error: detailsError } = await supabase
        .from('billings')
        .select('*, visit:visits(visit_date, patient:patients(first_name, last_name))')
        .eq('id', billing.id)
        .single();
      ...
      if (detailsError) {
        ...
        const cptCount = Array.isArray(details.cpt_codes) ? details.cpt_codes.length : 0;
        const icd10Count = Array.isArray(details.icd10_codes) ? details.icd10_codes.length : 0;
        logTest('Get billing details', true, `CPT: ${cptCount}, ICD-10: ${icd10Count}, Time: ${details.time_spent}min`);
      }
    }
    ...
    if (proceduresError) {
      logTest('Get procedure scopes', false, `Error: ${proceduresError.message}`);
    } else {
      logTest('Get procedure scopes', true, `Found ${procedures?.length || 0} procedure definitions`);
    }
...
async function testRLSIsolation() {
  ...
  try {
    ...
    if (tenantsError) {
      logTest('Tenants RLS (should work with service role)', false, `Error: ${tenantsError.message}`);
    } else {
      logTest('Tenants RLS (service role can see all)', true, `Found ${tenants?.length || 0} tenants`);
    }
    ...
    if (invitesError) {
      ...
      logTest('User invites RLS (service role can see all)', true, `Found ${invites?.length || 0} invites`);
    }
    ...
    if (rlsStatus) {
      const tenantsRLS = rlsStatus.find(t => t.tablename === 'tenants')?.rowsecurity;
      const invitesRLS = rlsStatus.find(t => t.tablename === 'user_invites')?.rowsecurity;
      const woundNotesRLS = rlsStatus.find(t => t.tablename === 'wound_notes')?.rowsecurity;
      
      logTest('Tenants RLS enabled', tenantsRLS === true, `Status: ${tenantsRLS}`);
      logTest('User invites RLS enabled', invitesRLS === true, `Status: ${invitesRLS}`);
      logTest('Wound notes RLS enabled', woundNotesRLS === true, `Status: ${woundNotesRLS}`);
    }
    ...
    if (userRolesRLS && userRolesRLS.length > 0) {
      ...
      logTest('User roles RLS disabled (by design)', isDisabled, `Status: ${userRolesRLS[0].rowsecurity}`);
    }
...
async function testAdmin() {
  ...
  try {
    ...
    if (facilitiesError) {
      ...
      logTest('List facilities', true, `${activeCount} active of ${facilities?.length || 0} total`);
    }
    ...
    if (rolesError) {
      ...
      logTest('List user roles', true, `Found ${roles?.length || 0} role assignments`);
    }
    
    // Test 4: Test RPC function
    if (users && users.length > 0) {
      const { data: roleInfo, error: rpcError } = await supabase
        .rpc('get_user_role_info', { user_uuid: users[0].id });
      
      if (rpcError) {
        logTest('RPC get_user_role_info', false, `Error: ${rpcError.message}`);
      } else if (roleInfo && roleInfo.length > 0) {
        logTest('RPC get_user_role_info', true, `User role: ${roleInfo[0].role}`);
      } else {
...
async function testPDFData() {
  ...
  try {
    ...
    if (addendumsError) {
      logTest('Get addendums for PDF', false, `Error: ${addendumsError.message}`);
    } else {
      logTest('Get addendums for PDF', true, `Found ${addendums?.length || 0} addendums with user data`);
    }
