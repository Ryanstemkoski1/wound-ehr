...
**Source:** Client meeting transcripts (February 20 + March 16, 2026)  
**Priority:** CRITICAL - AI note generation + treatment orders + mobile optimization  
**Status:** Phase 11.1 COMPLETE — Phase 11.6 (Treatment Order Builder) COMPLETE — Phase 11.7 (Client Forms) COMPLETE — Phase 11.2–11.5 next
...
3. **Treatment Order Builder** (tabbed sentence-builder UI — replaces Aprima treatment section)
4. Facility access control refinements (hide pending notes)
5. Clinical summary PDF templates (G-tube and wound care)
6. Mobile UI optimization for field use
7. PDF printing enhancements
...
### 11.6: Treatment Order Builder (5–7 days) ✅ **COMPLETE — Mar 16, 2026**

**Source:** Client meeting March 16, 2026 — Dr. May presented a tabbed treatment order concept to replace the Aprima wound form’s treatment section. Follow-up email confirmed: _“The 'Treatment Order' section I am proposing is intended to replace and simplify the 'Treatment Order' section of the Aprima assessment form.”_

**Business Value:**

- Completes the clinical documentation workflow (assessment → treatment → order)
- Replaces slow free-text entry with structured dropdowns and sentence builders
- Auto-generates readable treatment orders for nursing staff
- Currently the `treatments` table exists but has **zero UI** — this is the critical missing piece

**Reference Documents:**

- `TREATMENT MENU TABS.docx` (client attachment — 4 tabs with option lists)
- `1 Aprima Wound Form.pdf` (existing form being replaced)
- Client meeting transcript: March 16, 2026

---

**11.6.1: Database Migration & Types (Day 1, Morning)**

- [x] **Create migration `00029_treatment_wound_id.sql`**
  - [x] Add `wound_id UUID REFERENCES wounds(id) ON DELETE CASCADE` (nullable) to `treatments` table
  - [x] Add composite index on `(visit_id, wound_id)`
  - [x] Add `treatment_tab`, `cleanser`, `coverage`, `secondary_treatment`, `generated_order_text` columns
  - [x] Update RLS policies
- [x] **Regenerate TypeScript types** (`npm run db:types`)

**11.6.2: Treatment Option Constants (Day 1, Afternoon)**

- [x] **Create `lib/treatment-options.ts`**
  - [x] Define `TreatmentTab = "topical" | "compression_npwt" | "skin_moisture" | "rash_dermatitis"`
  - [x] Define `TreatmentOrderData` type (per-wound form state)
  - [x] **Tab 1 — Topical Treatment:** Cleansers (saline, water, Dakin’s, acetic acid, chlorhexidine, etc.), Topical treatments (wound gel, hydrogel gauze, oil emulsion, collagen, alginate, foam, Santyl, medical honey, silver products), Coverage (dry dressing, open air, duoderm, transparent film)
  - [x] **Tab 2 — Compression/NPWT:** 4 radio options (NPWT, UNNA boot, ACE wrap, multi-layer), pressure/frequency fields
  - [x] **Tab 3 — Skin/Moisture:** Cleansers, moisture barrier treatments, coverage options (open air, hydrocolloid, film)
  - [x] **Tab 4 — Rash/Dermatitis:** Cream/ointment options, frequency (daily, every shift, BID, TID)
  - [x] Sentence template builder functions per tab

**11.6.3: Server Actions (Day 2)**

- [x] **Create `app/actions/treatments.ts`**
  - [x] `createTreatment(formData)` — insert with `wound_id`
  - [x] `updateTreatment(formData)` — update existing
  - [x] `getTreatmentsByVisit(visitId)` — all treatments for visit display
  - [x] `getTreatmentForWound(visitId, woundId)` — per-wound lookup
  - [x] `autosaveTreatmentDraft(data)` — upsert pattern (same as assessment autosave)
  - [x] `deleteTreatment(treatmentId)` — cleanup

**11.6.4: Treatment Order Builder Component (Days 3–4)**

- [x] **Create `components/assessments/treatment-order-builder.tsx`**
  - [x] `"use client"` component
  - [x] Props: `{ visitId, woundId, existingTreatment?, exudateAmount?, onSave }`
  - [x] **Layout:** 4 horizontal tabs (shadcn `<Tabs>`)
  - [x] **Tab 1 (Topical):** Sentence builder — horizontal flow of text + `<Select>` dropdowns:
    - “Cleanse wound with `[cleanser ▼]`, pad dry, and loosely apply `[📦 treatment popup]` to wound bed”
    - “Cover with `[coverage ▼]` every `[N]` day(s) `[☐ PRN]`”
    - Optional secondary treatment line ("Add secondary treatment" button)
    - Treatment popup: modal/popover with categorized checkboxes
  - [x] **Tab 2 (Compression/NPWT):** Radio selection of 4 options + pressure/frequency
  - [x] **Tab 3 (Skin/Moisture):** Similar sentence builder + coverage section (open air, hydrocolloid + mfr instructions, film)
  - [x] **Tab 4 (Rash/Dermatitis):** Cream selection + frequency radio
  - [x] **Bottom:** Read-only “Generated Order” preview
  - [x] Wire in `validateTreatmentSelection()` from `lib/validations/assessment.ts`

**11.6.5: Integration & Display (Day 5)**

- [x] **Integrate into `multi-wound-assessment-form.tsx`**
  - [x] Add `<TreatmentOrderBuilder>` as new Card section after Infection Signs, before Assessment Notes
  - [x] Per-wound treatment state (switches with wound tabs)
  - [x] Save treatments alongside assessments in `handleSubmitAll()`
  - [x] Extend autosave to include treatment data
- [x] **Display on visit detail page**
  - [x] Add “Treatment Orders” card in right column (after Wound Assessments)
  - [x] Show per-wound treatment order sentences
  - [x] Edit link if visit not yet approved/locked
- [x] **PDF integration**
  - [x] Replace `assessment_notes → treatmentplan` workaround with actual treatment data
  - [x] Add treatment orders section to wound progress PDF

**Deliverable:** ✅ Complete treatment order workflow integrated into the clinical assessment
...
**11.7.1: Auto-Save Improvements (Week 8, Days 1-2)**
...
**11.7.2: Search & Filter Enhancements (Week 8, Day 3)**
...
**11.7.3: Notifications & Alerts (Week 8, Days 4-5)**
...
| -------------------------------- | ----------- | --------- | ------------------ |
| 11.1: AI-Powered Note Generation | 🔴 CRITICAL | 4 weeks   | ✅ Complete        |
| 11.6: Treatment Order Builder    | 🔴 HIGH     | 5–7 days  | ✅ Complete        |
...
| 11.7: Additional Polish          | 🟢 LOW      | 1 week    | Nice-to-Have       |
...
**Status:** Phase 11.1 + 11.6 + 11.7 COMPLETE - Phase 11.2-11.5 remaining
